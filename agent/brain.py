...
REGLAS:
1. NO SALUDES si el cliente ya te habló. 
2. PRIORIDAD AL CATÁLOGO: Si en los datos dice 'Amoladora TOTAL', usa ese nombre. 
3. PRECIOS: Dáselos EXACTOS si aparecen.
4. NUNCA menciones códigos ni proveedores.

DATOS DEL LOCAL (STOCK Y PRECIOS):
{contexto if contexto else "No hay coincidencias exactas. Asesorá técnicamente hasta que el cliente sea puntual."}
""".strip()
